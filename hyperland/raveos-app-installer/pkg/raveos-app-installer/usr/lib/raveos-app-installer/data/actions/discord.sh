...
#!/bin/bash
sudo -u "$USER" env HOME="$HOME" /usr/local/bin/VencordInstallerCli-linux -install || true
sudo -u "$USER" env HOME="$HOME" /usr/local/bin/VencordInstallerCli-linux -install-openasar || true
SCRIPT
